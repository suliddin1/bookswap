...
**The clean friends-only beta remains deployed, but a production-reported P0 iPhone listing-authoring exception is fixed only in the repository and guarded development environment; an authorized deployment and post-deploy mobile smoke are still required.** The failure was reproduced as an unguarded object-URL preview exception that escaped the form, and the same journey exposed an unhandled Supabase Auth lookup rejection. Both client boundaries are contained, Azerbaijani recovery is present, a fresh normal development user completed real login/upload/create/render with zero browser or HTTP errors, and Chromium plus iPhone WebKit regressions pass. No production data, deployment, or remote Git action occurred. Friend self-service Auth also remains blocked by external transactional email configuration. The owner-created clean beta project remains separate from the paused legacy and development projects; all 22 migrations and the established database/authorization gates remain unchanged. Public launch additionally remains blocked by recovery evidence, legal facts, administrator MFA/hosted controls, operations ownership, and field evidence.
