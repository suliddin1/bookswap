...
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| PROD-001 | Reconcile the 2-entry legacy production migration history with the 22 immutable repository migrations, then apply only after a tested backup                       | `bookswap` is confirmed, but production has 8 rather than 13 launch tables and contains one real account; blind `db push` or reset is unsafe |
...
| PRE-002 | Run an approved external dependency advisory query (`npm audit` or equivalent) and triage current advisories       | Patched installed-tree baseline passes; a live 28 July audit was blocked because sending the dependency tree to npm was not separately authorized |
