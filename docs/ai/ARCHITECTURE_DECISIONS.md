...
## ADR-033 - Private account responses fail closed before owned state changes

Status: Accepted.

Successful private account responses are untrusted until their complete render-critical shape is validated. Profile dashboard/load/save/status data, privacy-request history/submission data, and favorites collections must validate before replacing identity-owned client state. Profile fields are string-or-null as defined by the API; favorite counts are non-negative integers; privacy types/statuses match database constraints; timestamps are finite before deterministic formatting; and listing values reuse ADR-032's shared validator. A private client must not invent a false empty state, retain malformed mutation output, expose provider diagnostics, or throw because an HTTP 2xx body is incomplete.

This is a presentation defense in depth, not authorization evidence. Browser-local authenticated fixtures may prove parsing, identity-state containment, localized failures, focus, and reflow, but cannot close AUTH-03 or the protected route matrix without the authorized development configuration and server secret. Load-error focus that depends on a conditionally mounted alert must be keyed to both the error and the mount/readiness state so asynchronous batching cannot consume the only focus attempt before the target exists.

## ADR-034 - Listing authoring responses are resource-bound and lifecycle-safe

Status: Accepted.

Listing creation, edit load/save, image upload, and abandoned-upload cleanup treat successful HTTP status as necessary but insufficient. Create/edit responses must contain a complete listing accepted by ADR-032's shared validator; edit responses must also retain the requested route ID, and edit mutation metadata must use a strict boolean. Upload success must return exactly one unique HTTPS public `listing-images` URL per selected file with no credentials, query, or fragment. Cleanup success must account for every requested URL through non-negative integral accepted/referenced totals and expose a strict pending boolean. Invalid JSON, malformed success, wrong-resource data, provider diagnostics, and native errors settle into reviewed Azerbaijani failures before publication or client-state replacement.

Failure after upload still invokes the existing authenticated compensation endpoint. That endpoint's server-side project-host, owner-folder, and listing-reference checks remain authoritative, so a malformed create/edit response cannot authorize deletion and cannot remove an image already attached by a successful mutation. The client must not claim cleanup was queued when the cleanup response itself is unverified. These presentation contracts complement rather than replace Auth, route ownership, RLS, Storage policies, and the durable cleanup queue; browser-local fixtures prove fail-closed presentation and request sequencing only, not protected-route authorization.

Conditionally rendered failure targets use mount-time focus when effect scheduling could run before the target exists. A stable callback ref is preferred for this case because it executes when React attaches the node and does not depend on animation frames or background-worker timing. Every change to these authoring response shapes must update the shared parser, affected consumers, valid/invalid unit cases, permanent browser regression, and four-viewport failure evidence together.

## ADR-035 - Listing-detail action acknowledgements are resource- and context-bound

Status: Accepted.

An authenticated listing-detail action may change client presentation only after both HTTP success and an exact acknowledgement contract. Detail data must retain the requested route ID and a `sellerId` identical to the safe embedded seller. Room creation requires a valid room ID and the expected listing, buyer, and seller IDs. Favorite lookup/mutation requires the expected listing ID and a strict boolean equal to the requested state. Report creation requires a valid ID, expected listing/reporter IDs, `open` status, and finite timestamp. Review creation requires a valid ID, expected listing/author IDs, requested rating, the server-normalized trimmed comment, and a finite timestamp. A review ID already rendered for the current listing is not appended again.

Listing ID and authenticated user ID together define the client action context. Loads are abortable where the transport permits it; every load/mutation settlement also checks the latest context and request generation before navigation or state change. A context transition immediately hides prior listing data, clears resource/identity-owned drafts and feedback, and prevents an old completion from toggling a new listing, opening a room, or publishing status into another reader's view. Signed-out actions retain the explicit login path; failures for a reader already represented as signed in stay on the listing and use reviewed Azerbaijani copy derived only from stable error codes or `LocalizedClientError`, never provider/parser text.

Favorite and report route acknowledgements include the resource/user identifiers required for this check; review responses select only explicit render-critical columns. Unexpected listing/room lookup errors must propagate as internal failures rather than being misreported as absence or ineligibility. These response projections do not change authorization, eligibility, RLS, or durable mutation semantics. A malformed acknowledgement is not compensated by deleting a room, report, or review: room/favorite writes are idempotent and report/review uniqueness protects retries, so the UI fails closed and reconciles on a later load. Every contract change requires focused valid/invalid parser tests, a permanent authenticated browser regression, and four-viewport optimized-production failure evidence.

## ADR-036 - Shared listing-card favorites are acknowledgement- and context-bound

Status: Accepted.

Every `BookCard` favorite mutation used by home, catalog, seller, related-listing, favorites, and profile surfaces consumes the normalized favorite acknowledgement from ADR-035. HTTP success alone cannot toggle presentation: `data.listingId` must equal the card's listing and `data.saved` must be a strict boolean equal to the requested state. Saved state, failure status, pending state, and async settlement are keyed to the active listing/prop context so a reused component or stale completion cannot update a different card. Overlapping requests are guarded while the focused button remains mounted, exposes pressed/busy/disabled semantics, and retains a unique status relationship even when the same listing is rendered more than once.

The authenticated fetch boundary carries an internal stable `AUTH_REQUIRED` classification in addition to reviewed Azerbaijani copy. Only that explicit condition navigates to login. A non-OK, invalid-JSON, malformed, cross-resource, native, configuration, or other signed-in failure remains on the current surface and presents reviewed Azerbaijani text derived from a stable code or local fallback; provider and parser prose never renders. This is a presentation contract and does not change session validation, favorite authorization/visibility, API response shape, database mutation semantics, or parent collection reconciliation. Parser/unit evidence, one permanent authenticated failure regression, and optimized-production home/catalog/seller/related checks at all required viewports travel with changes to this boundary.

## ADR-037 - Profile listing mutations are acknowledgement- and identity-bound

Status: Accepted.

Profile-dashboard status and deletion actions may change identity-owned presentation only after an exact successful acknowledgement. A status response must contain a complete listing accepted by the shared validator, the requested listing ID, both normalized seller identifiers equal to the current authenticated owner, the requested status, and strict boolean image-cleanup metadata. A deletion response retains its existing deletion and cleanup fields, adds the deleted listing ID, and requires the exact ID, literal `deleted: true`, and a strict boolean cleanup value before the card is removed or a cleanup notice is shown. A wrong-resource, wrong-owner, opposite-state, incomplete, invalid-JSON, or boolean-like response fails closed into reviewed Azerbaijani copy without removing or replacing listing state.

Authenticated user ID, listing ID, and request generation together own pending state and settlement. Only one dashboard listing mutation may be active at a time; authored controls remain mounted and focusable with guarded `aria-disabled` semantics. Identity changes invalidate pending work immediately, and an old request cannot clear a newer busy state, publish feedback into another account, replace a different listing, or remove a card no longer present in the current owner collection. This client contract complements rather than replaces protected route ownership/ban checks, relisting moderation, RLS, Storage cleanup, and the durable image queue. Browser-local fixtures prove failure containment and presentation only, not protected authorization.

## ADR-038 - Account mutation acknowledgements are requester- and intent-bound

Status: Accepted.

Profile saves and privacy-request submissions may change identity-owned presentation only after an exact successful acknowledgement. Both mutation envelopes include the authenticated `requesterId`; the client requires it to equal the current Auth user. A profile acknowledgement must also contain exactly the normalized name, phone, and city submitted by that request. A privacy acknowledgement must contain the submitted request type, the initial `open` status, a valid renderable timestamp, and the current requester while continuing to omit the sensitive request details from the response. Structurally valid data for another user, another intent, or another profile value fails closed into reviewed Azerbaijani copy without replacing profile state, resetting the form, appending history, or exposing diagnostics.

Authenticated user ID and request generation own each pending mutation and settlement. Identity changes abort the transport where possible, invalidate old work, clear private form/status state, and prevent an old completion from changing the next account. Duplicate submission is guarded while the native control remains mounted and focusable through `aria-disabled`. Privacy history loading merges any request submitted after that load began instead of overwriting it. These presentation guarantees complement rather than replace route authorization, service-role checks, RLS, privacy workflow constraints, or the blocked live protected-route matrix. Changes to either acknowledgement require focused valid/adversarial parser cases and browser coverage at all four required viewports.
