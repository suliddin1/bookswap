...
export async function moderateText(text: string) {
  ...
  if (!process.env.OPENAI_API_KEY) return { safe: true, reason: "Demo moderation passed" };
  ...
  return { safe: !body.results?.[0]?.flagged, reason: body.results?.[0]?.flagged ? "Text was flagged" : "Passed" };
}
